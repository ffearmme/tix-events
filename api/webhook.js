...
async function sendTicketEmail(destEmail, newTickets, orderId) {
    ...
    try {
        ...
        if (result.error) {
            ...
            console.log("✅ Ticket email sent via webhook:", result.data?.id);
        }
    } catch (err) {
        console.error("❌ Unexpected error sending ticket email in webhook:", err);
    }
}
...
async function sendMerchEmail(destEmail, merchItems, orderId) {
    ...
    try {
        ...
        if (result.error) {
            console.error("❌ Resend API Error (Merch) in webhook:", JSON.stringify(result.error, null, 2));
        } else {
            console.log("✅ Merch email sent via webhook:", result.data?.id);
        }
    } catch (err) {
        console.error("❌ Unexpected error sending merch email in webhook:", err);
    }
}
